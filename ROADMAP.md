...
4. **Onboarding v2, screens** (split 2026-09-08 along the twelve screens in RULINGS; master is not live, so a half-built flow may land and the next half continues it).
   ...
   - 4a-i-b. [ ] **Weight has one source and no copy** (the redesign Kd asked for on 2026-09-10 after the post-merge review of 4a-i found a High: the profile form echoing a weigh-in's number left a "typed by me" entry the person never typed, so deleting the mistaken weigh-in kept its number). Migration `0027` drops the `users.weight_kg` cache after re-running 0026's safety backfill; every reader computes the weight from the history; the sync code, its shortcuts and the lock essays are gone; the reviewer's thirteen small findings fixed in the same pull request. No screen changes, so no click-through (RULINGS 2026-09-09). *Built 2026-09-10, branch `weight-one-source`.*
