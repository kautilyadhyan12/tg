...
4. **Onboarding v2, screens** (split 2026-09-08 along the twelve screens in RULINGS; master is not live, so a half-built flow may land and the next half continues it).
   ...
   - 4a-iii. [ ] **One daily-calories number: the macro rings read the plan.** **Ask Kd first** (found in 4a-ii's review): protein is 2.0 g per kg on a loss, 44.2 % of the golden loss plan's calories, above the IOM's 10–35 % for the general public — (a) keep it and say on screen that it is a high-protein plan (recommended: sports-nutrition guidance for people who train is 1.4–2.0 g per kg), or (b) cap protein at 35 % (carbs are then 40 %, still under the IOM's 45 %). The nutrition targets route (`/v1/nutrition/targets`, still shown by the web's macro rings) moves onto the stored plan answers — today it reads the old profile with a fixed −400/+300 and can differ from the number screens 6–7 show by a few hundred kcal; the one-element `fitness_goals` mirror 4a-i writes ends there, and with it the one gap it leaves open — the v1 PUT replaces that array without touching `main_goal`, so until the rings move the two can disagree for anyone who edits goals on the old form. Someone who finished the old form has no "your day" answer, so the rings' honest empty state must send them to the question that is missing. Comes BEFORE 4a-iv: the rings must read the plan before the goals move underneath them.
   - 4a-iv. [ ] **Many goals, and a gym.** Screen 1 becomes ONE weight choice (lose · keep · gain — it alone sets the number) plus "also work on", any number (RULINGS 2026-09-10). A `weight_goal` column with its CHECK replaces `main_goal` by migration, backfilled by the very mapping the code applies today (weight loss → lose, build muscle → gain, the rest → keep), so nobody's number moves; the old goal joins the "also" list unless it was weight loss. `fitness_goals` becomes the "also" list, its enum losing weight loss and gaining get stronger and better balance; the v1 profile PUT and Settings' goal chips take the same two questions; `PLAN_GOAL_BY_MAIN_GOAL` goes, the direction is stored, never derived. "Build muscle" ticked raises protein to at least 2.0 g per kg; nothing else in the calorie maths reads the "also" list. Screen 7 gains "A gym (everything there)" beside the home equipment, a new equipment value with its CHECK (Kd said yes, RULINGS 2026-09-10, decision B).
