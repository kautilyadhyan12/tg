...
- **Round one: 3 High, 4 Low, 5 weak tests — every one fixed, each High first written as a failing test.** H-1 the fill re-filled a slot the gym had cancelled (its check counted running dates only); H-2 Stop and Remove deleted cancelled dates, so a new repeat wrote them fresh — now they keep them, and such a date holds its class off that whole day and cannot be put back; H-3 the browser's Back showed a stale list. L-1 the list lost its width (my PowerShell swallowed the `${…}`), L-2 a refusal now re-reads the week, L-3 a time the clocks skip is refused. New: a two-server race test (lock removed: **28 of 28 answered 200, two classes at 19:00 on all 14 dates**, 3 of 3 runs), last-week dates by a fixed clock on every weekday and in Auckland, a Back-button test, the real `errorText`. api **97 files, 2,814 passed, 0 failed**; a normal night of the fill 1,075 ms against master's 1,482.
- **The re-check closed all of round one and found N-1, a High my H-2 fix made**: the kept day held its class off for good — nothing could lift it, an extra 07:00 class never ran there, and the refusal said the class no longer repeated on a weekday it did. Now putting such a day back **lifts the hold** (the row goes, the class's live repeats on that weekday write the day, an audit row), and with no such repeat it says *"No Spin repeats on Tuesdays now …"*; the screen says what the hold does and offers *"Let Spin run this day"*. Test first, seen red.
- **The second re-check closed N-1 and found N-2 (High) and N-3 (Low)**: both new sentences were untrue in ordinary cases (a class already running that day; a repeat that starts later), and a second hold on the same date left a lift reporting success with nothing running. Now the lift clears every hold of that class on that date inside a savepoint and rolls back when no running class is added, saying *"No Row repeat would add a class on Tuesday 29 September, so this day stays as it is."*; the panel reads *"… no Spin repeat adds a class on this day."* Tests first, seen red.
- **Next in Folder B**: the confirmation in the same reviewer terminal, then merge on Kd's word, then 17b-ii-b-ii — this day and later, and every repeat of one class at once.
