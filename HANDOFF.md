...
- **Verified after round one's fixes: api tsc 0 · api eslint `src test tools` 0 · shared tsc 0 · shared eslint 0 · engine purity grep silent · the whole api suite on the local Postgres and the real Redis 2,762 passed, 95 files, 0 failed** · shared 213 passed · **eighteen deliberate breaks, all RED** (rows 43–60 of `apps/api/tools/mutate-member-list.mjs`, and 3a-v-a's row 35 refreshed) · `--census` **61/61 anchors** before and after, every file restored and sha256-checked; seven older anchors were refreshed where this job moved the code they pointed at. Six older tests changed because this job REVERSES what they asserted (a person coming off is kept, not deleted) and three because the shapes grew a field.
