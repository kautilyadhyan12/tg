...
- **Open for Kd**: Resend's block list covers the whole account, so a person who marks an invitation as spam stops getting sign-in codes too. To switch on in production: Resend webhook → `RESEND_WEBHOOK_SECRET`, a Full access `RESEND_API_KEY`, `OPERATOR_EMAIL`. Next in Folder A: 3b-ii.
