...
- CI green on 58b25c6 (run 35120584443, the Neon migration check skipped: no migration). Kd's click-through: steps 1–6 passed; the dropdown was a native `<select>`, whose list Windows draws white — now `common/Select.jsx` (Card 7's dark list), which opens upward where its list would run off the window (headless Edge at 1280×800 and 400×800 showed it off-screen, then on it); `select.render.test.jsx` 3 tests, its break (never upward) red. Next: steps 7–9, the §6 review (RUN it: a request's measure, another person's dish), merge on Kd's word; then 7a-iv-b.
