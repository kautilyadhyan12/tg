...
## 2026-09-22 · 3a-v-b: the gym's whole file is KEPT, nobody is deleted, and a query string stops reaching the log (PR #92)

- **The job** (Folder A's second, Opus xhigh, risky). Spec §11.1, §11.4, §11.5. Migration `0037` (written as `0035`, renumbered twice as Folder B's jobs merged first): nine columns on an entry — the gym's membership and payment words, three dates, the end-or-renewal KIND, the gym's own columns as one document, `former_at`, and the field NAMES staff edited by hand — plus `gym_member_list_fields`, the gym's own catalogue of its own headings. New file `memberList/extraFields.ts` (pure); `memberList.keep.unit.test.ts` and `memberList.kept.routes.test.ts` new.
- **NOBODY IS DELETED FROM A GYM'S LIST ANY MORE**, which reverses §9.2 rule 2. A person the file no longer holds is marked `former_at`; because `(gym_id, identity_key)` already covered a former row, the upload that holds them again REVIVES that very row with its id and everything that will hang off it (visits, reports). `returning` says how many of the new are not new to the gym. An already-former record is in no `gone`: marking it again would write a fresh date over the day they really left.
- **A former record is off the list in FOUR separate places**, and each is its own deliberate break: the rule's `current`, `membersAgainstList`'s two channels, the chips, and a page. `canBeInvited` is the number 3b's Invite button acts on, so a leak there is an ex-member emailed a way back into a gym they left.
- **"Changed" is a list of FIELDS now** — counted field by field, so 412 changed cannot hide 412 dates read the wrong way round off one column — **and a field the file has no column for is LEFT ALONE**. That reverses 3a-iii-b, where a file with no status column emptied every status word the gym had; a gym exporting a narrower report has not stopped keeping the columns it omits. The gym's own columns are merged by key for the same reason.
- **The hand-edit tick is a SECOND question, not a second reason for the first.** One is "this file is the wrong file", the other "this file is right and your own staff's work is lost anyway"; a yes to one is not a yes to both, and a test drives all four combinations. The refusal carries a count and field NAMES in plain English, never a value and never a person, and only the marks the file really overwrote are cleared. The screen that SETS a mark is 3a-iv's; this job writes the column, the rule, the tick and the clearing so that card is written once.
- **§11.2 RUNS AGAIN AT THE BOUNDARY THAT WRITES — this job's worst-thing line.** 3a-v-a drops a card-shaped cell in the reader, but a staged upload lives an hour as a document in the database, outlives a deploy that tightens a rule and can be reached by a hand-run statement. A card-shaped cell is dropped and counted, never refused, and its KEY is still written empty — leaving the key out would let the jsonb merge keep the very cell the write was dropping. **One limit is PINNED rather than discovered**: the cell rule catches a cell that IS a card, not a sentence with one in it, which is §11.2's own words; widening it would blank a Notes cell one time in ten for holding a long number. Written into §11's notes for the feature's hostile security pass.
- **A LEAK OLDER THAN THIS JOB, found by its own log capture and closed.** pino's stock `req` serializer writes a url as it arrived — path AND query string — so `GET …/entries?query=<a member's address>` put that address in the log, and `?status=` has been on that route since 3a-iii-b. Nothing drove it because the capture only ever drove the upload routes, whose values travel in a POST body. `logSafety.ts` now replaces the `req` serializer too: the path is kept, the query string is not, globally rather than route by route.
- **And a test that could never have failed.** The leak capture's status sentinel was `Frozen-${uuid}` — 43 characters, cut to 40 on the way in — so `written.includes(sentinel)` could not have matched however loudly the server printed it. Found because this job's two new word filters refused one as too long. All three word sentinels now fit inside the 40.
- **Verified after round one's fixes: api tsc 0 · api eslint `src test tools` 0 · shared tsc 0 · shared eslint 0 · engine purity grep silent · the whole api suite on the local Postgres and the real Redis 2,780 passed, 95 files, 0 failed** · shared 213 passed · **twenty-two deliberate breaks, all RED** (rows 43–64 of `apps/api/tools/mutate-member-list.mjs`, and 3a-v-a's row 35 refreshed) · `--census` **65/65 anchors** before and after, every file restored and sha256-checked; seven older anchors were refreshed where this job moved the code they pointed at. Six older tests changed because this job REVERSES what they asserted (a person coming off is kept, not deleted) and three because the shapes grew a field.
- **Cost at full size, into spec §9.9** (2026-09-22, 2,592 MHz **on mains** — a first set taken on battery was thrown away and Kd plugged the laptop in), three runs at each of three sizes, 10,000 people × 30 of the gym's own columns. **The READS did not get worse** — a page at the cap is 66–87 ms of answering nobody where the five-field list was 69–147 — and the three new filters cost what the status filter already cost. **The CONFIRM roughly doubled**: 2,270–3,040 ms at ten thousand where it was 980–1,635, and 3,027–3,982 ms at the cap. At the launch shape it is unchanged (128–231 ms). A gym confirms about once a month and presses the button itself; the pool of ONE connection is Stage 4 item 11. One number worth watching that is not a millisecond: **a file of 10,000 people with 30 of the gym's own columns is about 4 MB against a 5 MiB ceiling**.
- **ROUND ONE: four Highs and three Lows, every one fixed and driven.** The reviewer reproduced every High against the real service before reporting it. **Three were this card's own rules not holding where it said they did**: the gym's catalogue was grown and INSERTed BEFORE the two tick gates, and a gate `return`s out of `sql.begin`, which COMMITS — so a confirm that printed "NOTHING was changed" had already written the file's headings into a bounded resource nothing prunes, which staff could exhaust with files the guard refuses; the end-or-renewal KIND was compared without its day, so everybody with an EMPTY end cell read as `changed` on every upload for ever, bumping the version and clearing hand-edit marks for a field nothing overwrote; and **the card-in-a-sentence limit this card PINNED was reported anyway, rightly** — two live-format card numbers read back out of the database under members' names, which is the plan's own worst-thing line. The fourth was 3a-v-a's `column_<place>` key, made durable here: inserting a column to its left made the same cell a SECOND catalogue field, both labels empty, nothing clearing the stale one. Lows: the preview tool printed its warnings twice; a former record always said "not in the app", false about a real person on the page whose job is to show them; the extra document bounded a key and a value but not how many keys.
- **AND ONE MORE I FOUND MYSELF, by looking at a real preview on a page instead of at a test**: round one's card-in-a-note fix scrubbed the ROW's cells and left the three SAMPLE cells staff see beside each heading — which are not only on a screen, they are stored in the staged upload's document for the hour it lives. That is the same fault 3a-v-a's own round one called a Critical. **And breaking 3a-v-a's older row for that rule then ran GREEN**: its mutant drops the `!card` guard, and with a scrub behind the guard a whole-cell card came out redacted anyway — a rule guarded twice cannot be disproved by breaking it once. That row now takes out both halves.
- **AND CHASING THAT FOUND THREE MORE OF THE SAME CLASS** — the answer to a question I had put in my own re-check prompt. The status, the membership word and the payment word are all cleaned through `cleanStatus`, and a name through `cleanName`, and only a WHOLE-cell card was ever dropped from any of them: "paid by card 4111 1111 1111 1111" in a payment column reached the staged file intact. Both cleaners now scrub, which is the boundary where a raw cell becomes a kept value, so the row, the samples, the words and the name are one rule. **The member NUMBER keeps its whole-cell rule on purpose** — its whole job is to be a long number, which is where a false positive costs a gym its own data — and that is asserted rather than assumed.
- **Second re-check: three open, all in the card-in-text rule, all fixed (`12bf226`).** The group-span rule altered 202 of 203 weight logs and cost 45.8 ms a cell inside the confirm; it is now issuer prefix + card grouping + Luhn, linear, measured at 546/546 test cards caught, 0-1 % of random number lists altered and a 200-person number-log confirm at 128-229 ms (was 22.0 s). The scrub ran after the cut, so a 40-character word with a card came out at 42 and the upload answered 500; it now scrubs first and cuts once.
- **THE ROUND IS CLOSED** (third re-check, `reviews/3a-v-b-4-verdict.md`): nothing Critical or High open; the reviewer drove every fix through the real service. Its one note, older than this job — the whole-cell card rule has no issuer-prefix check, so a Luhn-valid number no issuer uses is dropped alone and kept beside a word — is filed on ROADMAP item 10 as its own defect.
- **The lesson of the round, and it is mine.** My first deliberate break for the unnamed-column fix pointed at a table test that builds its fields BY HAND and never reaches the rule in `understand.ts` — so the mutant ran GREEN and read exactly like a rule that holds. Found only by running it. The reviewer had separately named four tests that would stay green with the thing they check broken, and was right about all four; each is now driven, including the same WIDE file confirmed twice with an empty end cell, which the old case could not see because it used the narrow fixture.
- **Kd decided one thing (RULINGS 2026-09-22)**: who counts as the same person does not change, so a gym that corrects a spelling leaves a FORMER record beside the new one — put to him with its alternative and the recommendation, and *"go"*. The case to settle is the person's page (3a-iv). **PR #92 is open and NOT merged; round one's re-check prompt is `reviews/3a-v-b-3-recheck.md`** — he has not said "merge". Round one's review file is `reviews/3a-v-b-1-review.md`. Then 3a-iv. The four dashboard images on disk are still nobody's. **One full-suite run failed `orgs.routes.test.ts`'s SETUP and passed it alone and on the next full run — that is ROADMAP item 10's recorded shared-database flake** (nine files seed one database), the same shape as `auth.routes.test.ts`'s on 2026-09-16, and nothing this job touches: its own suite uses `mkept-t-%` users and an INR plan of its own.
