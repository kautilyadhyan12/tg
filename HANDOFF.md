...
- **The re-check closed the round** (`-4-verdict.md`: all five closed, nothing broken; the reviewer's race scripts re-run, 0 of 10 each where round one had 4 and 10). Open: Kd's "merge". 3b-i-b's "two workers at once" test failed four CI runs and passed two, never locally (ROADMAP 10); `42b826f` makes a failure print each report's state and each run's outcome, and that run passed. Next in Folder A: 3b-ii-b.
