...
- **NOT yet run against Paddle's real sandbox**: it needs Kd's free sandbox account (API key, client-side token, webhook secret, default payment link `https://localhost/`). The review must run it for real.
...
- Next: Kd's sandbox account, click-through, round-one review. Re-seed every database after merge (the rupee prices).
