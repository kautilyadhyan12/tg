...
- **Not done yet, first thing for the next chat**: set up Folder B (`git worktree add ..\ai-home-gym-b`, its own local database and Redis number, `corepack pnpm install --frozen-lockfile --offline`, a note file naming its list), give Kd the start text for each folder, and push branch `replan-2026-09-21` so the first build jobs can branch from it (nine commits of records, not pushed — Kd has not been asked).
