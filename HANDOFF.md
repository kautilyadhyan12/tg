...
## 2026-09-23 · 3b-i-b: what comes back (Folder A, branch `invite-results`)

- **Built** (risky, Opus xhigh; Kd said *"go"*, nothing new decided, so no RULINGS line): `POST /v1/webhooks/resend` (Svix signature on the raw bytes, 5-minute window, kept once by `svix-id`, only type · Resend's id · bounce type stored); the worker's `invites.results` job acts on a report only after `GET /emails/{id}` agrees; a hard bounce or `email.suppressed` keeps the address from every gym, a complaint from that gym; a gym's first 50 then wait (all back, or an hour); a gym stops past 2 % bounces (counted as at least 50) or on a complaint in its first 100, its waiting emails skipped as `sending_stopped`; `OPERATOR_EMAIL` told once; `tools/invites-stopped.ts` lists and resumes. Migration `0039`. No new package. Spec §9.12 "Out of 3b-i-b".
- **The worst thing, first tests**: a report nobody at Resend signed (other secret, 6 minutes old, one byte changed, unsigned) changes nothing; a signed one Resend's record does not bear out stops nobody; after a confirmed complaint that gym cannot send again; after a confirmed hard bounce no gym invites the address.
- **Three deliberate breaks of the core rule, each RED, restored sha256-identical**: any signature accepted (`[200, 401, 200, 401]` for `[401 ×4]`); the confirmation skipped (two suppressions written where none should be); the complaint's suppression dropped (`[]` for one row).
- **Outside cases**: Svix's published test vector verifies (and 16 near-misses do not); Resend's published `email.bounced` example reads as a hard bounce.
- **Verified**: api tsc 0 · api eslint 0 · shared tsc 0 · shared eslint 0 · shared 220/220 · results unit 153/153 · results routes 15/15 · invite routes and unit 111/111 · full api suite on local Postgres 105 files, 3,257 passed, 0 failed · web 2,466 passed (`poseAssets.contract` fails locally on Node 24 only, as before). No harness targets a file this job changed.
- **Cost** (mains, 2,592 MHz): a report arriving 36 ms median (150 worst), mostly the commit; the gate 10 ms at the launch shape, 13 ms with a gym of 10,000 sent; a report at that gym 127 ms in the worker, a same-gym list write waiting 61 ms median (104 worst).
- **Open**: round-one review (`reviews/3b-i-b-1-review.md`). To switch on in production: Resend webhook → `RESEND_WEBHOOK_SECRET`, a Full access `RESEND_API_KEY`, `OPERATOR_EMAIL`. Next in Folder A: 3b-ii.
