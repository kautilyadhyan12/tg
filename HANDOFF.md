...
- Verified: api tsc 0 · eslint src test tools 0 · scan unit 82/82 · foods unit 23/23 · measures unit 111/111 · scan routes 33/33 on the loaded table and on `aihg_ci_scratch` · full api 1381/1381 in 68 files · 21 deliberate breaks each red (scratchpad `breaks.ps1`, `breaks2.ps1`), the files restored identical (sha256).
...
- Next: push, CI, the re-check of the last fix in both reviewer chats (prompts in this chat), then #80 merges, then #81, and one click-through of both. The four dashboard images are still not this card's.
