...
- Found building (spec §9.4 amended): Node 24.11.1's `TextDecoder("windows-1252")` reads 0x80–0x9F as control characters (27 bytes against Python's cp1252: € ’ “ ” Š š Ž ž Œ œ Ÿ), so 1252 is our own table; Excel's Mac CSV ends its last line in CRLF ("at least as many" bare CRs); declared XML over 25 MiB → `too_complex`; Strict Open XML reads as .xlsx; the ZIP64 locator check was dead code (the contiguity rule refuses it) and is gone.
...
- Measured (spec §9.9): Excel's own 10,000 × 30 .xlsx 1,268 ms, peak RSS 111 → 246 MB; CSV 493 ms, 99 → 159 MB; longest request-thread stall 21 ms.
