...
- **Verified after round one's fixes: api tsc 0 · api eslint `src test tools` 0 · shared tsc 0 · shared eslint 0 · engine purity grep silent · the whole api suite on the local Postgres and the real Redis 2,759 passed, 95 files, 0 failed** · shared 213 passed · **fifteen deliberate breaks, all RED** (rows 43–57 of `apps/api/tools/mutate-member-list.mjs`) · `--census` **58/58 anchors** before and after, every file restored and sha256-checked; seven older anchors were refreshed where this job moved the code they pointed at. Six older tests changed because this job REVERSES what they asserted (a person coming off is kept, not deleted) and three because the shapes grew a field.
