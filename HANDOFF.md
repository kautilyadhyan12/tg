...
- **Next: the re-check, in THAT SAME reviewer terminal** (CLAUDE.md §6, never a fresh one): `reviews/3a-v-a-3-recheck.md` is written; Kd types `Re-check reviews/3a-v-a-3-recheck.md` there. PR #90 is open and not merged — Kd has not said "merge", and the ROADMAP line’s merge date assumes today. Then 3a-v-b (keeping it: one migration, the reconcile rule over the wider row, former records, the filters). The four dashboard images on disk are still nobody’s.
