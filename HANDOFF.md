...
- **Verified: api tsc 0 · api eslint `src test tools` 0 · shared tsc 0 · shared eslint 0 · engine purity grep silent · the whole api suite on the local Postgres and the real Redis 2,716 passed, 93 files, 0 failed** · shared 213 passed · **nine deliberate breaks, all RED** (rows 43–51 of `apps/api/tools/mutate-member-list.mjs`) · `--census` **52/52 anchors** before and after, every file restored and sha256-checked; seven older anchors were refreshed where this job moved the code they pointed at. Six older tests changed because this job REVERSES what they asserted (a person coming off is kept, not deleted) and three because the shapes grew a field.
...
- **Kd decided one thing (RULINGS 2026-09-22)**: who counts as the same person does not change, so a gym that corrects a spelling leaves a FORMER record beside the new one — put to him with its alternative and the recommendation, and *"go"*. The case to settle is the person's page (3a-iv). **PR #92 is open and NOT merged** — he has not said "merge". Round one's review file is `reviews/3a-v-b-1-review.md`. Then 3a-iv. The four dashboard images on disk are still nobody's.
