...
- **The line was split first.** 17b was three tables, the repeat rule, a nightly worker AND a week calendar — more than one terminal and more than a reviewable diff. ROADMAP now has **17b-i** (this job: class types, repeats, the dates, the Classes screen) and **17b-ii** (the week view, and changing a single day); 17c's "Needs" re-pointed to 17b-i. Kd said **go**; he decided nothing new, so no RULINGS line.
- **What a gym can now do**: console → **Classes**. Add a class (name, minutes, places or "no limit", a coach from its own staff, one of eight colours, an open-gym tick), edit it, take it off the timetable, and say when it repeats (weekdays, the gym's clock time, from/until). **Saving a repeat writes its dates at once** — the fill runs in the same transaction — and each repeat shows its next four dates and how many it holds.
- **The one rule, and it is kept in the database**: a repeat is the gym's CLOCK TIME plus `gyms.timezone`, and each date's instant is `(local_date + make_interval(mins => local_start_minute)) AT TIME ZONE g.timezone`. Measured, not assumed: 18:00 London is `17:00+00` on 29 Mar 2026 and `18:00+00` on 25 Oct — six o'clock on both. The gap hour (01:30 on 29 Mar) and the doubled hour (01:30 on 25 Oct) both keep their class; both answers are in `fill.ts`'s header with the session that produced them.
- **Safe to run twice is the DATABASE's guarantee**: one `INSERT … SELECT … ON CONFLICT (schedule_id, local_date) DO NOTHING` against a unique index, never `DO UPDATE` — so a retry, two workers, or a save landing on the nightly job all leave one calendar, and a day the gym changed on purpose (`changed_alone`, 17b-ii's flag, shipped here because the fill is what must respect it) survives every later run.
- **New**: migration `0035` (three tables + a TENTH privilege `schedule.manage`, backfilled onto owner and manager rows only — §13.3's words, and the difference from `0019`'s all-roles backfill) · `modules/orgs/classes/{fill,repo,service,routes}.ts` · a nightly worker job at 05:00 UTC (daily, not hourly: a HORIZON has no 02:00, unlike the rollup) · `packages/shared/src/classes.ts` · `pages/console/Classes.jsx` + `classesView.js`. `TimePick` moved out of `OpeningHoursPanel` into its own component rather than being written twice — behaviour unchanged, its 27 tests green.
- **Two things corrected in passing.** `tenancy.ts`'s privileges CHECK was TWO behind the deployed one (it never gained `attendance.read` from `0019`), so a regenerated schema would have narrowed a live constraint — fixed with the comment saying why nothing caught it. And `gym_class_types` / `gym_class_sessions` joined `USER_LINKED_NOT_PURGED_TABLES` on the day they were created (the coach is the only `users` link in the three tables).
- **Deliberately narrower than spec §13.3, said out loud**: a REPEAT carries only WHEN. The length, the places and the coach live on the class, because two places to store one length is one edit away from a screen and a calendar disagreeing. A different coach on Wednesday is a second class type, or 17b-ii's per-day change. The column that would widen it is absent rather than present and unwritten.
- **Proven by running** (pasted in the terminal): api tsc 0 · api eslint `src test tools` 0 · shared tsc/eslint 0, 193/193 · engine 213/213 · **the whole api suite on the local database: 90 files, 2,296 passed, 0 failed** · web 2,385 passed. **Three deliberate breaks on the core rule, each red and put back** (checksums matched afterwards): the gym's zone swapped for UTC → the 6pm class reads `18:00Z` in BST, an hour late; `DO NOTHING` → `DO UPDATE` → the second run rewrites all 9 rows and the horizon roll writes 57 instead of 1; the gym dropped from the edit's `WHERE` → a rival gym's owner gets 200 where they must get 404.
- **Cost at full size, 20 gyms × 60 classes = 19,200 dates** (`.cost/cost17b.mts`, this laptop at full clock): the nightly fill 1,459 ms cold and a **167–283 ms median warm, in the WORKER, not the api** · reading one gym's whole timetable 16.3 ms median · **the only part that blocks the api's event loop — shaping and serialising — 1.2 ms median, 3.2 ms p95, 28.5 KB** · saving one repeat 80.8 ms median under that gym's row lock. The save's fill was scoped to its own repeat after measuring: p95 159.9 → 93.8 ms. For scale, an EMPTY transaction here is 8.7 ms and one that merely takes the gym's row lock is 38.6 ms, so the save is the Docker volume's fsync rather than work this card added.
- **Two pre-existing failures, neither this card's, both reproduced on a clean tree**: `apps/web/src/hooks/poseAssets.contract.test.js` is a vitest transform SyntaxError on Node 24 (the file is unmodified, valid UTF-8, and `node --check` parses it; CI runs Node 22 and master is green), and `pnpm typecheck` at the ROOT cannot run here at all — turbo shells out to the machine's global pnpm 11, which refuses under corepack. Per-package runs are the proof, which is what CLAUDE.md §5 prescribes.
- **Kd clicked through it, passed it, and then changed three things** (second commit on the branch, before any review). (1) He read *"Dates are written 8 weeks ahead"* and asked **"will gyms set things 8 weeks ahead?"** — which is what the sentence made him think and the opposite of what happens. Checked against the market: neither Mindbody nor TeamUp tells a gym anything about its own generation window. **The number was right and the sentence was wrong** — struck, and with it the count on an OPEN-ENDED repeat, which read "16 dates on the calendar" where 16 is the window's size, not how many times the class runs. It says **ongoing** now; a repeat the gym gave an end date keeps its count, because there it is true. (2) **Bring back**, on his ruling, after being shown that TeamUp *"will not be able to reinstate"* an archived Class Type and Mindbody restores one with an Activate button; its repeats stay stopped and the nightly fill is driven to prove they stay that way. (3) **Nothing that destroys dates does it on one tap** — Remove and Stop both ask IN PLACE (`ConfirmInline`, new in `ConsoleStates`, never a modal: *"not covering whole screen"*), which is the class of his complaint rather than the one button he named.
- **Driven in a real browser over CDP before he was asked to look** (Folder B, `127.0.0.1`, the script is in `apps/api/.cost/`): add a class → add a repeat → "From Tue 22 Sep 2026 · ongoing" → Remove asks → Keep it leaves it → Remove it archives → "No longer running · 1 kept" → Bring back returns it live with no repeat. Screenshots kept. After the changes: api tsc/lint 0, shared 193/193, engine 213/213, **the whole api suite 90 files / 2,297 passed**, web **2,388 passed**.
- **Next**: the round-one review (`reviews/17b-i-1-review.md`), then **merge** on his word. After it, 17b-ii.
