...
- Next: the re-check of THESE fixes goes back to that same reviewer chat (resume it; the round is still open). Then the two extra passes — hostile security, then data integrity — each in its own fresh chat. The four dashboard images on disk are still not this card's.
