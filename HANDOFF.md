...
- Next: the re-check goes back to **that same reviewer chat** (CLAUDE.md §6), not a fresh one, on commit `<this one>`. Then Kd's word to merge. The four dashboard images on disk are still not this card's.
