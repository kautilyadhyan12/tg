...
- Next: **the two extra passes CLAUDE.md §6 asks for on a card that touches other people's data** — hostile security, then data integrity, each in its own FRESH chat, both after this round rather than instead of it. Then Kd's word to merge, then 3a-iii-b (whose confirm inherits the recorded hazard: Postgres puts the whole failing row into a CHECK violation's `detail`). The four dashboard images on disk are still not this card's.
