...
- **Proven by running**: api tsc 0 · eslint 0 · shared tsc 0 · eslint 0 · 202/202 · **the whole api suite on `aihg_b` with the real Redis: 95 files, 2,701 passed, 0 failed** (an earlier run, with the web suite running beside it, failed one staff test that took 5,013 ms; it passed alone and in the clean re-run) · web 2,440 passed (`poseAssets.contract` fails locally on Node 24 only, as before).
