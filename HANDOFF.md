...
## 2026-09-21 · 3a-iii-a: PR #87's review round one fixed (3 High, 4 Low, 4 weak tests), after a crash mid-commit
...
- Lows: the one shipped sentence claiming a preview is always computed afresh is struck; neither read route had the validation-failure test §4 requires (a table of nine now); an identity key travelled in an Error message Sentry captures (gone); `listState` was read twice in one request (hoisted).
