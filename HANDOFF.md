...
- **Part 2, the member record, is agreed (*"REST I AGREE"*) and recorded**: RULINGS 2026-09-21 (one line) · spec Part 3 §11, new, reversing §9.2 rule 7 and amending §9.11 · ROADMAP 3a-v-a and 3a-v-b, built BEFORE 3a-iv. A gym's file is kept whole — ten standard fields plus the gym's own extra columns — minus a never-keep list (bank cards by check digit in ANY column, bank details, government IDs, passwords and PINs, medical notes). **One design call in §11.1 is the chat's and still has to be put to Kd, with Part 3: a person who leaves becomes a FORMER record, not a deleted one** (visits and reports need the history).
...
- Next: **Part 3, check-in** — the front-desk scan, the double scan, the live log, what replaces the member's tap — put to Kd in simple words, then written down.
