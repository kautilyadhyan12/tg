...
- Tests: the time limit proves a terminate (exit code 1 via the `onWorkerExit` seam; a worker ending itself exits 0); the crash path (`workerAnswer.ts`, split from `parseWorker.ts`) answers a class name only, never a message, and a name shaped like a cell is `unknown`; the row cut counts the rows handed over (one past the cut); the bomb through the real worker is `too_complex` with this thread's heap under +100 MB; the four slow shapes through the worker answered in under 5 s.
