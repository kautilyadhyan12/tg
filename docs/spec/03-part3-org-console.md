...
| | 20 gyms of 200 (the launch shape) | 10,000, the biggest a list may hold |
|---|---|---|
| confirm — the first, which writes the whole list | 289–350 ms · 18–25 ms · 2 ms · **183–184 ms** | 1,228–2,075 ms · 41–59 ms · 1 ms · **980–1,635 ms** |
| confirm — next month's file, every status moved | 129–542 ms · 16–23 ms · 2 ms · **82–298 ms** | 1,948 ms · 47 ms · 1 ms · **1,496 ms** |
| `GET /` — the list and its status chips | 23–53 ms · 5–15 ms · 1 ms · **11–40 ms** | 40–63 ms · 12–15 ms · 1–2 ms · **18–27 ms** |
| `GET /entries` — a page of 100 | 25–54 ms · 7–16 ms · 1 ms · **8–36 ms** | 29–30 ms · 9 ms · 1 ms · **11–12 ms** |
| `GET /entries` filtered, or searched | 21–52 ms · 6–16 ms · 1 ms · **9–37 ms** | 28–45 ms · 9–14 ms · 1 ms · **11–26 ms** |
| walking every name, a page at a time | 48–104 ms, 2 pages · **10–39 ms** | 3,187–3,198 ms, 100 pages · **22–25 ms** |

**THE READS ARE CHEAP AT BOTH SIZES, AND THAT IS THE DESIGN AND NOT LUCK.** The kept list is a TABLE with an index on it, so a page of the biggest list allowed costs a bystander 11–12 ms and walking all hundred pages costs 22–25 ms — where the PREVIEW's page of the same ten thousand people costs 226–334 ms, because that one cuts a page out of a ten-thousand-person document. They are different routes answering different questions and the table above is not a comparison: the preview's cost is the price of showing a file nobody has agreed to yet.

**THE CONFIRM IS THE EXPENSIVE ONE, AND AT TEN THOUSAND IT COSTS THE SERVER ABOUT A SECOND AND A HALF OF ANSWERING NOBODY.** It is one transaction that writes the lot — the rule again under the lock, one INSERT, one UPDATE, one DELETE, the stamp, the audit row — on the ONE connection the whole API shares, so while it runs no other gym and no member gets past. At the launch shape it is 183–184 ms, which is comfortable. At ten thousand it is not comfortable and it is written down rather than tuned away: it is **ROADMAP Stage 4 item 11, the pool of one**, and it is bounded by how rarely it happens — a gym confirms a list about once a month, and presses the button itself, so nobody is waiting on a schedule. Nothing in this card can make one connection into two.
