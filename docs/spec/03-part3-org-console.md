...
- **The name each joiner signed up with**: the Members screen's roster carries, for staff holding `members.confirm`, the name on the list of the record the membership was joined through and `matches` or `differs` (`invites/nameCheck.ts`). A wrong "differs" only asks staff to look, so every doubt answers it: a name still the one a code sign-up was given from its own address is no name (round one's C1 — a mistyped address is usually built from the member's name); every part of the signed-up name must be found in its own part of the list's — the same word, a shortening of three letters or more ("Rob" for "Robert", never "Shahani" for "Shah"), or an initial either way; and at least two of the list's parts (its only one, if one) must be matched by words, one of them the same word, so a first name alone is asked about. Order, case, accents and punctuation are ignored; an apostrophe joins a name. Its cases are W3C's "Personal names around the world", round one's, and names made from mistyped addresses. Remove is the roster's existing one tap.
