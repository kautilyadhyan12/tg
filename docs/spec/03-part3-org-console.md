...
- A typed field is tidied, then goes through the file's own rule for it (sign-in's email rule, the phone reader with the gym's country, the member-number rule, the card rule); one that cannot be kept refuses the whole change with the server's sentence, and a card number typed into any field is refused (§11.2). The digit group straight after a "+" begins a phone number and never a card — in typed fields and in a file's cells alike, since +49151… mobiles with no spaces pass Luhn about one time in ten — and only that group is exempt, so a card joined on after a phone is still found. The phone box always asks the card rule with an issuer prefix (never true of a number starting 0 or 00), and skips the check-digit-only rule for a number valid for its country: 0049, 0044 and 0091 numbers pass that one about one time in ten. Digits typed with neither "+" nor a leading 0, such as 4915100000400, are still refused when shaped like a card; they would have been read as a different number anyway.
