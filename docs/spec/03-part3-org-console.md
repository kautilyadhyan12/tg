...
- The worker (queue `invites`, every minute) takes one email at a time under an advisory lock: a gym 500 in any 24 hours, 200 on trial; the app `INVITE_EMAILS_PER_DAY` (default 2,000); `INVITES_PAUSED` stops it. Just before each email it checks again (`decideSend`, a table test over every case): the gym active, on a plan, with an address; the invitation pending; a current record holding the address; not in the app; not suppressed; a valid, unshared address; the domain takes mail (MX, else an address record; a null MX or no domain is no mail; a resolver failure retries). Resend's `Idempotency-Key` is the send's id, so a retried send is one email; a send whose worker vanished more than 20 hours ago is failed, never sent. 401 and 403 from Resend retry rather than spend an invitation.
- The email: the ruled subject, From "{gym} via AI Home Gym" on `INVITE_EMAIL_FROM`, the gym's name and city cleaned and cut at 60, its postal address, the join link `{WEB_ORIGIN}/join/{slug}` (its page comes with 3b-ii), and the unsubscribe link with both RFC 8058 headers. In production invitations are off unless `INVITE_EMAIL_FROM`, `INVITE_HMAC_SECRET` (never to change once set) and `API_ORIGIN` are all set.
- Cost at full size (2026-09-23, mains, 2,592 MHz; the api has one database connection, so a bystander's worst wait is what counts). At the cap, 10,000 records and 2,000 app members: the count 147–176 ms (429 ms once, the first request of a fresh server), a page of 100 names 141–173 ms, a page filtered by invitation 135–218 ms, one person's page 6–13 ms — each led by the whole-gym member match (161–174 ms alone), the statement the list page already runs. Pressing Invite for 9,000 people: 142–174 ms in two of six runs and 798–943 ms in the other four; a trace of eight more (in batches of 200) put every long wait — 651–1,392 ms, in seven of them — on one batch write that stalled, where the others took about 47 ms. Written in one transaction the press held everybody for 1,176 ms and its write alone took 713–792 ms. Plain 200-row inserts with no app code stalled for 746 ms on this machine's Docker database in the same session, so the stalls are the disk, not the batches. The launch shape, a gym of 200: the press 87–154 ms, "Add and invite" 82–125 ms, a page 11–12 ms. The worker, which is not the api: 85–129 ms an email, most of it the claim and the finish, each its own commit (36–46 and 31–34 ms).
