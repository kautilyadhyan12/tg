...
**Measured 2026-09-21 as §4's "cost at full size" rule asks, through the real HTTP server, with a bystander's `GET /health` fired beside each call** — because the API's Postgres pool is ONE connection (`app.ts`), so a request waiting on the database is also a request nothing else gets past, and the event loop alone would be the flattering number. Three runs at the launch shape and two at ten thousand, on a laptop whose power plan was holding the processor at **1,495 MHz of its 2,592 MHz rating**; the same harness at full clock the night before measured about half of every figure below, so the shape of the answer is the code's and its size is the machine's:

| | 20 gyms of 200 (the launch shape) | 10,000, the biggest a list may hold |
|---|---|---|
| upload → preview | 1.3–2.3 s wall · 9–15 ms event loop · bystander 12–13 ms | 4.2–4.4 s · 165–180 ms · 61–72 ms |
| read the preview back | 46–108 ms · 1–4 ms · 8–17 ms | 209–225 ms · 47–48 ms · 8–9 ms |
| one page of 100 names | 62–129 ms · 3–7 ms · 10–14 ms | 601–802 ms · 12–18 ms · 10–11 ms |
| page 50 of the list | 66–106 ms · 1–4 ms · 7–13 ms | 556–621 ms · 19–24 ms · 5–6 ms |
| walking every name | 142–280 ms, 2 pages, worst block 4–16 ms | 68–70 s, 100 pages, worst block 37–38 ms |

A page's WALL clock is not what improved — the work moved from Node to Postgres, not out of existence. What changed is that it is now several small statements rather than one long one, so the single connection is free between them and the bystander is answered while the page is still being built: **8–17 ms beside every read here, against the 309 ms the review measured beside a page of the first version.** At the launch shape the request thread never stops for more than 7 ms on a read, or 15 ms on an upload. **The pool of one is the remaining ceiling and it is not this card's**: ROADMAP Stage 4 item 11.

Two limits found while measuring, both recorded rather than fixed here. **A preview still blocks this thread for 165–180 ms at ten thousand people** (60 ms at full clock), which is the rule running over every row on the request thread; it cannot move to the worker, which has no database, and at twelve uploads an hour a person it is accepted. **And `postgres.js` refuses more than 65,534 parameters in one statement**, so 3a-iii-b's confirm writing a row per person would fail at about 8,000 people — measured, and the reason §9.7's `INSERT … SELECT FROM jsonb_to_recordset($1)` is the only shape that reaches the biggest list allowed, not a style choice.
