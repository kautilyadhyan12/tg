...
- Migration `0039`: `gyms.invites_stopped_at` / `invites_stopped_reason` (`bounces` · `complaint`) / `invites_counted_from`; `gym_invite_sends.result` (`delivered` · `bounced` · `complained` · `failed`, only on an email that went) and `result_at`; `webhook_events` gains `attempts` and `not_before`.
- `POST /v1/webhooks/resend`: the raw bytes (no parser runs first), Svix's signature (any `v1,` of the header, constant-time, the timestamp within 5 minutes), kept once by `svix-id` in `webhook_events` as `{ type, emailId, bounceType }` only — never the address or the subject — then 200. Kept: `email.delivered`, `bounced`, `complained`, `failed`, `suppressed`; every other signed event is answered 200 and dropped. No secret set: 503, so Resend keeps the events. Its own rate limit, 1,200 a minute.
- The worker (queue `invites`, job `invites.results`, every minute) acts on a report only for an invitation email (Resend's id on a sent row; a sign-in code's report is let go) and only once `GET /emails/{id}` agrees: `last_event` is the report's own event or a later one (delivered is confirmed by opened, clicked or complained). Not caught up yet (queued, sent, delivery_delayed): asked again at 1, 5, 15, 60 and 180 minutes; still not after 8 tries, or Resend has no such email: given up, logged, nothing done. A 401/403 stops the run and says the key needs Full access.
- What a confirmed report does: a delivery is recorded; a bounce Resend types `Permanent` (or leaves untyped), and `email.suppressed`, keep the address from EVERY gym; a `Transient` or other bounce is only "didn't arrive"; a complaint keeps it from that gym. An email keeps its most serious result (delivered < failed < bounced < complained). A person's page shows the result beside the email.
- A gym's first 50 emails go and the next waits until all 50 have a result or an hour after the 50th went (`mayGymSend`, decided in the claim). A gym stops when bounces × 50 > the larger of emails sent and 50 — its second bounce in the first 50, three in 100, five in 200 — or on a complaint about one of its first 100 (`judgeGym`). A stop skips its waiting emails as `sending_stopped` (nothing spent: a later press queues them again), and an email that may already have gone ends as `send_unknown`. Staff see `blocked: "sending_stopped"` and a press, Invite, send again or Add and invite answers 409 `sending_stopped`.
...
- Cost at full size (2026-09-23, mains, 2,592 MHz, local database, the api's one connection): one report arriving 36 ms median, 150 ms worst (mostly the commit); the claim's gate 10 ms median for 20 gyms of 200 due, 13 ms (18 worst) with one gym of 10,000 sent beside them; one report acted on at a gym of 10,000 sent 127 ms median in the worker, during which a list write to that gym waited 61 ms median, 104 ms worst.
