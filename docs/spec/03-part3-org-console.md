...
- **The name each joiner signed up with**: the Members screen's roster carries, for staff holding `members.confirm`, the name on the list of the record the membership was joined through and `matches` or `differs` (`invites/nameCheck.ts`): the name with fewer parts must be found part by part in the other — the same word, its start of three letters or more, or its initial — with at least one whole word; order, case, accents and punctuation ignored. A nickname, another script or a shared first name is not caught, which is why both names are always shown. Its cases are W3C's "Personal names around the world". Remove is the roster's existing one tap.
...
- **A plan of one's own**: each open invitation at a gym with a live plan carries `yourPlan` for a person with a live plan of their own — what it gives beyond the gym's member plan (`ownPlanExtras`, the terms `mergeEqualRank` uses; the switched-off coach never named) and where to cancel it (the App Store or Google Play for a store purchase, else "where you bought it").
