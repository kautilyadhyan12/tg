...
| 11 | As **account B** (the manager), open the console → **Members**, and look at the **Waiting to join** section. | It says how many are waiting, then **"Nobody can be let in until this gym is on a plan. The people waiting keep their place."** — the second sentence is new in this card. **Confirm** and **Not this person** are both greyed out. ⚠️ **AND READ THE GREY LINE UNDER C's NAME, which is the half this step used to miss:** it says **"Waiting ‹N› days"** and **NOTHING ELSE.** There must be **no "Expires in ‹N› days" and no "Due to expire"** anywhere on that row — a deadline printed one line under "keep their place" is the same lie this card removed from C's own screen. |
