...
PROVE → in Claude Code the chat RUNS the commands itself (typecheck, lint,
        scoped tests, greps, gitleaks, mutation sweeps) and pastes the GENUINE
        output, then fixes and re-runs until green. It never declares success on
        output it has not seen, and never summarises output in place of pasting
        it. (~~a chat cannot run your repo; YOU run them and paste the output
        back~~ — true of the browser workflow this playbook was written for, not
        of Claude Code, where it has a terminal.) `apps/api`'s DB-backed suites
        are the one exception: they need Docker Desktop running, so the chat
        asks Kd to start it once per machine restart, then runs them itself.
...
2. After coding, **run the verification commands yourself** (typecheck, lint, scoped tests, relevant greps) and paste the genuine output in full; iterate on real output only. (~~end with the commands for Kd to run and wait for the pasted output~~ — browser-workflow text; see PROVE in Part I §2.) Describing expected results as if they were observed is an automatic rejection, and so is summarising output instead of pasting it.
...
# APPENDIX — Verification commands (the chat runs these itself and pastes genuine output; Kd re-runs them for money/auth/engine per Part VI §3. `pnpm -w typecheck`/`-w lint` fail at corepack's version check — run per package.)
