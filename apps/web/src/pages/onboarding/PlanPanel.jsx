...
// the note is the first sentence of. `targetKg` is the target on screen, which
// the healthy-weight line is held against as screen 3 holds it.
...
export default function PlanPanel({ plan, direction, units, targetKg = null, note = true }) {
  const [open, setOpen] = useState(false);
  if (plan === null) return null;
  const target = targetLine(plan, units);
  const flags = flagLines(plan, direction, units, targetKg);
  return (
