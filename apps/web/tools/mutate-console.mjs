...
const MUTANTS = [
  ...
  // ── KD'S CALENDAR (2026-09-03, `:31508`) ────────────────────────────────
  // Every row below sits in rule 4a's "numbers a user sees" column: a wrong
  // square is this app telling somebody they came on a day they did not, or
  // did not come on a day they did.
  {
    id: 'C191',
    target: 'attendancepanel',
    suite: MY_GYMS_SUITE,
    why: "THE CALENDAR OPENS ON THE READER'S MONTH INSTEAD OF THE GYM'S, which is the client half of the defect the server card was arranged to prevent. A visit is filed under the GYM's day, so a member in Honolulu looking at an Assam gym late on the 30th gets a grid for the wrong month - the window and the squares answering differently about one visit. It is invisible for anybody in their gym's own zone, which is everybody who tests it by hand",
    expect: 'opens on the GYM s month even when the reader s calendar says another',
    from: '    ? monthKeyOfDay(gymZone === null ? gymToday(undefined, tick) : gymToday(gymZone, tick))',
    to: '    ? monthKeyOfDay(gymToday(undefined, tick))',
  },
  {
    id: 'C192',
    target: 'memberattendanceview',
    suite: MEMBER_ATTENDANCE_VIEW_SUITE,
    why: "THE MONTH ASKS FOR AN EMPTY WINDOW: `to` stops being the FIRST OF THE NEXT MONTH and becomes the first of this one, so from equals to and a half-open window holds nothing. Every month draws blank, and because the read succeeds there is no failure anywhere to notice - the honest-looking answer is that this member has never been to their gym",
    expect: 'asks for a half-open window whose end is the next month s first day',
    from: '  const next = shiftMonthKey(key, 1);',
    to: '  const next = key;',
  },
  {
    id: 'C193',
    target: 'attendancepanel',
    suite: MY_GYMS_SUITE,
    why: "THE STAMP GOES AND ONE MONTH'S VISITS ARE DRAWN UNDER ANOTHER'S HEADING (:29250 §6). A slow August answer landing after the member has stepped back to September paints August's days onto September squares - days they never came, on a screen whose whole subject is which days they did. The ordering that makes it happen is a slow request, so it is rare, invisible in a fast test and permanent once seen",
    expect: 'never calls a month empty while its read is still in flight',
    from: '  const fresh = history.forMonth === month && month !== null;',
    to: '  const fresh = month !== null;',
  },
  {
    id: 'C194',
    target: 'memberattendanceview',
    suite: MEMBER_ATTENDANCE_VIEW_SUITE,
    why: "AN EMPTY MONTH GOES BACK TO CLAIMING THE MEMBER HAS NEVER COME - :4267 F1 and :4355 F4 exactly, a condition caused by OTHER months printed as a sentence about this one. The list this replaced could say it truthfully because it held everything; a month grid cannot see another month, so stepping back past the month somebody joined tells them they have never been to the gym they are standing in",
    expect: 'names the month rather than claiming the member has never come',
    from: "  return current === true ? 'No visits yet this month.' : `No visits in ${label}.`;",
    to: '  return "You haven\'t marked yourself in here yet.";',
  },
  {
    id: 'C195',
    target: 'attendancepanel',
    suite: MY_GYMS_SUITE,
    why: "THE FORWARD ARROW STOPS BEING BOUNDED, so a member can page into months that have not happened and read a grid of empty squares for November. Nothing is false on screen exactly, but they are shown a month the gym cannot have attendance for and given no reason why it is blank - and every press spends the shared 600/hour read bucket on a question with no answer",
    expect: 'will not step past the month the gym is in',
    from: '                disabled={currentMonth !== null && month >= currentMonth}',
    to: '                disabled={false}',
  },
  {
    id: 'C196',
    target: 'attendancepanel',
    suite: MY_GYMS_SUITE,
    why: "THE MARK'S CLOCK STOPS WINNING AND THE READ'S STALE PAIR DRAWS THE NEW CHIP (T3 round 1's L-4, undone). This is the defect the calendar CREATED and a probe caught: gating the history read on the hours read makes it land AFTER a quick tap, so its whole object replaced the clock format and a gym that had just switched to 12-hour spelled the new visit the old way. Holding the pair apart is what makes the precedence a fact rather than a race",
    expect: 'draws the new chip on the clock the MARK came back with',
    from: '  const shownClock = markedClock?.clockFormat ?? history.clockFormat;',
    to: '  const shownClock = history.clockFormat;',
  },
  {
    id: 'C197',
    target: 'attendancepanel',
    suite: MY_GYMS_SUITE,
    why: "A DAY CANNOT BE OPENED, so the times are gone from the product. Kd was told the cost of a grid in as many words - a square cannot show that somebody came at 5:01 PM and 3:32 AM, so the times move behind a tap - and a tap that does nothing is the calendar quietly losing what the list it replaced could show",
    expect: 'opens a day to its times',
    from: '                      if (cell.came) setOpenDay(cell.date);',
    to: '                      if (false) setOpenDay(cell.date);',
  },
  {
    id: 'C198',
    target: 'attendancepanel',
    suite: MY_GYMS_SUITE,
    why: "AN OPENED DAY SURVIVES A MONTH CHANGE, so the sheet sits over the screen headed by a date that is no longer in the month behind it - and its times belong to a month the member has stepped away from. The state holds a DATE rather than a row precisely so this cannot go stale silently, and clearing it on the step is the other half of that",
    expect: 'does not re-open a day by itself when the member steps back to its month',
    from: '    setOpenDay(null);\n    setMonthStep(shiftMonthKey(month, delta));',
    to: '    setMonthStep(shiftMonthKey(month, delta));',
  },
  {
    id: 'C199',
    target: 'attendancepanel',
    suite: MY_GYMS_SUITE,
    why: "A MONTH TRUNCATED BY THE SERVER'S PAGE LIMIT IS DRAWN AS IF IT WERE WHOLE. Past a hundred visits in one month the days beyond the first page simply do not appear, so the grid tells somebody they did not come on days they did - and it says nothing, which is worse than a caption because there is no way for them to know",
    expect: 'says so when a month holds more visits than it can show',
    from: '              {gridStatus === \x27ready\x27 && history.more ? (',
    to: '              {false ? (',
  },
  {
    id: 'C200',
    target: 'attendancepanel',
    suite: MY_GYMS_SUITE,
    why: "A FAILED MONTH IS DRAWN AS AN EMPTY ONE - the :8267/:8343 class, on the surface that answers 'which days did I come'. A dropped request becomes thirty blank squares and the sentence 'No visits yet this month', which is this app telling a member their own past is empty because a read did not land",
    expect: 'says the month could not be read, and never draws it as an empty month',
    from: "          {gridStatus === 'failed' ? (",
    to: '          {false ? (',
  },
  {
    id: 'C201',
    target: 'memberattendanceview',
    suite: MEMBER_ATTENDANCE_VIEW_SUITE,
    why: "FEBRUARY LOSES ITS LEAP DAY, so a member who came on 29 February has no square to be marked on - a visit the server returned and the grid cannot draw. It is the one month a hand-rolled length gets wrong and the one day that only exists every fourth year, which is exactly how long it would take anybody to notice",
    expect: 'counts the days of a month, leap years included',
    from: '    return [31, leap ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][parsed.month - 1];',
    to: '    return [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][parsed.month - 1];',
  },
  {
    id: 'C202',
    target: 'memberattendanceview',
    suite: MEMBER_ATTENDANCE_VIEW_SUITE,
    why: "THE MONTH STARTS UNDER THE WRONG WEEKDAY: one extra blank shifts every square by a day, so the fire lands beside the date it belongs to rather than on it. Every number is still correct and every column is wrong - the shape of a defect nobody reports because the grid still looks like a grid",
    expect: 'offsets the first day to its own weekday, Monday first',
  ...
  },
  {
    ...
    why: "NUMBER ON SCREEN: a one-day streak is printed beside a long week streak, so an owner reads 5 weeks in a row - 1 day in a row, two TRUE figures arranged into a sentence that reads as a contradiction. This is :30624's class, which is this exact screen's recorded defect from one card earlier, found by Kd and not by 1659 tests. It is also the meaning of the word: a streak of one is not a streak",
  ...
  },
  {
    ...
    target: 'onarollview',
    suite: ON_A_ROLL_VIEW_SUITE,
    why: "NUMBER ON SCREEN: the WEEK figure is built from the DAY field, so a member who has come for five weeks and three days running is shown to their owner as 3 weeks in a row. Kd ruled BOTH units precisely because they answer different questions, and a fixture where the two move together cannot see this - which is how C155 passed under its own mutant on this screen one card ago",
    expect: 'reads each figure from its own field',
    from: "    weeks: weeks === 1 ? '1 week in a row' : `${weeks} weeks in a row`,",
    to: "    weeks: days === 1 ? '1 week in a row' : `${days} weeks in a row`,",
  },
  {
    id: 'C217',
    target: 'onarollview',
