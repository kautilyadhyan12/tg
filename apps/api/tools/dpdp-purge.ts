...
try {
  ...
  await sql.end();
  // Non-zero exit is what an operator (and a cron) needs to see. Two ways the
  // run fell short, matching the worker entrypoint exactly (T3 round 3, F4):
  // a user's transaction threw (errors), or a leaderboard snapshot the scrub
  // cannot certify made this run withhold every marker (schemaDriftSnapshots).
  const shortfall = result.errors > 0 || result.schemaDriftSnapshots > 0;
