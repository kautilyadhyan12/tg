...
/** True when a member of the gym is matched (§9.7's own rule: proved address first,
 *  stated phone second) to one of the current entries holding this address. The
 *  one-person invite and the worker both ask it this way; the press asks it for the
 *  whole list at once. */
export async function addressInApp(
  ...
  for (const phone of new Set(holders.map((holder) => holder.phone))) {
    const reached = await membersAgainstList(sql, gymId, { email, phone });
