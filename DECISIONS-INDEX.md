...
what a future chat must READ is one constant and one warning, while the reasoning
that produced three different answers is a card record.** **Its most valuable
phrase guards against a tidy-up rather than a build** — *"before 'tidying' the
three different SEVENS in this feature into one constant"* — the second time
`:35762`'s coincidence trap has been armed on this one card. **THE FIFTY-FIRST IS
A KD RULING PLUS A CORRECTION OF THE
...
title**, which is the whole reason its four phrases are worth their always-read
cost: three of them stop a later chat quoting the tick as browser evidence for
things nobody saw. **THE FORTY-SEVENTH IS AN ADDENDUM RECORDING A GATE
...
clauses duplicating the entry's** — the generator deduplicating, measured not
assumed. Taken with this header's own prescribed command against the bytes being
committed. **THE THIRTY-THIRD IS A T3 ROUND THAT DOES NOT SHIP ITS
...
line added ZERO triggers because its phrases duplicate the entry's, **which is
the generator deduplicating rather than a line being lost** — measured, not
assumed. **THE TWENTY-NINTH MEASUREMENT IS A SMOKE GATE DISCHARGED
...
afternoon, at +5, +6, +3 and +5**, which is the split holding at a steady rate on
a day that produced two cards, a smoke declaration and a review round. Taken with
this header's own prescribed command against the bytes being committed, never by
adding a delta to the figures above. **THE TWENTY-FIFTH MEASUREMENT IS A SMOKE DECLARATION AND
...
is a CARD record and went to §1B, where :24813 sends it. Compare the +34 above,
which was four rulings. **THE FOURTEENTH MEASUREMENT ANSWERS THE QUESTION THE
...
- **:36816** — 2026-09-07 — **KD MOVES THE QUIET WINDOW A SECOND TIME IN ONE DAY
  ...
  three different SEVENS in this feature into one constant, before writing a
  fixture that names a literal number of quiet days, and before quoting `:36694`
  ruling 1's seven — it lasted about an hour.** *"if a user does not come to gym
  for say 3 continous day then gym can send not 7 … now start building"*.
  **§1 · `SLIPPING_AWAY_QUIET_DAYS = 3`**, the SECOND overrule of this number in
  one day (14 → 7 → 3, the card's recommendation losing twice), **so the constant
  is treated as VOLATILE: every fixture is written `SLIPPING_AWAY_QUIET_DAYS ± 1`
  and never as a literal.** **The cost he has not been shown, worked out and
  recorded: at three days a TWICE-A-WEEK member is on the list permanently**
  (Tue/Sat leaves Wed·Thu·Fri empty every week); bounded because nothing sends
  itself and the cap is one a week. Raised in one line, NOT made a second gate
  (`:16702`, `:19256`). **§2 · THREE SEVENS, ONE MOVED — `:35762`'s trap, second
  time on this card:** the quiet WINDOW (moved), the nudge's rolling `1/member/7d`
  CAP (Part 3 §4.1, unmoved), and the message EXPIRY (**derived from the CAP**,
  unmoved). **Had the expiry been derived from the window this ruling would have
  changed it silently.** **§3 · THE MESSAGE EXPIRY IS APPROVED** — *"yes i
  agree"* — fixing slice 1's cheer that draws FOR EVER (*"212 days ago"*), TRUE
  and so never a blocker, which is why it sat until Kd asked; a WEB change.
  **§4 · THE `:26777` GATE IS OPEN** — *"do the things planned"* covers this
  document's file list, migration and tests **as stated**; a departure goes back
  to him, **and the SQL still reaches him before any database does** (R4.4).
- **:36694** — 2026-09-07 — **KD RULES THE QUIET WINDOW AT ONE WEEK, AND HIS
  ...
  ANSWERING.** ⚠️ **ITS RULING 1 (seven days) IS SUPERSEDED THE SAME DAY BY
  `:36816` — THREE. Everything else here stands, §§1, 3 and 4 included.** **Read before building the at-risk list, before writing an empty
...
- **:36392** — 2026-09-06 — **THE CHEER'S GYM-DAY CAP: SMOKE PASSED AND THE 🟡
  ...
  member's side, before quoting it as covering the dot's EXPIRY, before asking Kd
  to re-run a sheet he has ruled already run, and before writing "the smoke
  passed" anywhere.** He asked for *"only relevent one … not the same tests over
  and over"*, so the run handed to him was **six checks on the four surfaces the
  cap change touches**, with the already-passed steps named as not re-run and
  why. **The first check exists only because of the ruling: both members had been
  cheered the day before and both rows had their four emoji BACK** — dead under
  the rolling seven days, freed by the gym's midnight — **so the half nobody can
  smoke by waiting was observable without waiting.** He reported *"all passed each
  and everything"*, then ruled the follow-up out: *"these tests were performed
  much before i dont want to do again"*. **The gate is therefore a DECLARATION**
  (`:27415`, `:32498`, `:33265`, `:34048` are the precedent) **and what sits under
  it is not nothing:** a real cheer written through the console at 23:29 his time,
  **exactly ONE row for that member on the gym's day**, audit clean — measured,
  not taken from the report (`:34147`). **NOT covered by the tick:** the member's
  card and dot for the person cheered tonight — **the sheet named the second row
  and the cheer landed on the FIRST**, which the database identified and the
  report did not — and the dot's expiry, which needs a day-old cheer. **Pressing
  row one retired the "reaches the row you pressed" check, so `C220` was run on
  today's bytes and is RED** rather than citing the older human run. **A `tm` card
  showing a dot would mean the dot is outliving its window; a `user` card is
  consistent with everything** — question recorded, not resolved, no `OWED.md`
  line (`:30733`'s shape). **And the sheet's step 11 was a FIFTH copy of the dead
  seven-day figure**, found while handing it over, after three in code and ten in
  the card document.
...
- **:34666** — 2026-09-04 — **"ON A ROLL" AND THE CHEER, T3 ROUND 2 (diff-only):
  ...
  claim inside a numbered list, before naming a tool in a PROVE section without
  naming its SCOPE, before quoting a mutation figure without the WEEKDAY it was
  measured on, before narrowing a write door's population without re-reading the
  sentence it refuses with, and before trusting a clamp that no schema stands
  above.** Six Low, all fixed; the escape hatch did not fire. **The `OWED.md`
  line still does NOT tick — no screen, so no smoke.** Also here because it is
  recorded nowhere else: **CI's full-history gitleaks went RED on `0108449`**,
  over three prose sentences reading *"API half: `kcalPointForSetsV…`"*, and
  `55395d0` pinned three fingerprints — **closing an `OWED.md` line that had sat
  DONE and unticked**, and whose own text predicted this.
...
- **:19366** — 2026-08-26 — **A GYM CAN FINALLY FIX ITS OWN DETAILS (server
  ...
  privilege to `ORG_PRIVILEGES`, before writing `gyms.country` or
  `gyms.currency_display`, and before adding a mutant to a schema the create and
  edit doors now SHARE.** Builds the 🟡 line raised at :14570; stage 1 of
  :19016's order. **KD RULING, one word at the plan gate: a NEW privilege
  `org.manage`, "edit gym details", OWNER ONLY** — the offered alternative
  (reusing `staff.manage`, no migration) was recommended AGAINST on :13803's
  precedent and he took the recommendation. **Implemented as owner-only BY
  DEFAULT, which is what the recommendation said**: it is in
  `ROLE_PRIVILEGES.owner` and in NEITHER `OWNER_ONLY_PRIVILEGES` nor
  `LAST_OWNER_REQUIRED_PRIVILEGES`, so an owner may tick it across (:11429 rule
  3) and cannot lock themselves out; a test drives the widening. §2.2 has no row
  for it — an ADDITION with no governing § (:9809's class).
  **THE FINDING: `gyms` had NO `country` COLUMN.** The wizard has collected it
  since 2026-08-18, `createOrg` maps it to a currency and the answer evaporates —
  so "change your country" would be a box that can be written and never read.
  Migration `0014` adds it, **DELIBERATELY NOT BACK-FILLED**: USD/CAD/GBP/INR
  each invert to exactly one row of `COUNTRY_CURRENCY` so a fill looks exact, but
  **INR is also `currency_display`'s own DEFAULT**, so the fill would stamp `IN`
  onto gyms that never chose it (EUR is ambiguous 20 ways regardless). NULL means
  "we never asked" — :10010/:10099's refusal of exactly this inference. Own ⚪
  line; each gym self-heals on its first save. The column's CHECK is SHAPE ONLY,
  because the SUPPORTED list lives in code and grows.
  **`0014` ALSO BACKFILLS THE NEW PRIVILEGE ONTO EVERY EXISTING OWNER, and that
  is NOT what Kd ruled against at :15381.** The snapshot rule protects a decision
  somebody MADE; nobody has ever decided anything about a privilege that does not
  exist until the migration runs. Without it `privilegesFor` prefers the stored
  set and **every existing owner is 403'd on their own gym** — the card ships
  dead, since the button that would re-grant it belongs to the unbuilt
  custom-names card. Same shape and same named R4.4 departure as `0013`'s.
  **Decisions not to re-derive: PATCH not PUT** (absent ≠ null — omit `city` and
  it is untouched, send null and it clears; C26's class, mutant **O119**) ·
  **`.strict()` REFUSES `currencyDisplay`, `slug`, `orgType` and `locale` rather
  than stripping them** (a refusal tells an owner we decide the money; a strip
  lets them watch it vanish) · **the country is checked BEFORE the transaction**,
  so a refusal leaves the row untouched rather than half-applied · **ONE
  `resolveCurrency` for both doors**, which WIDENED O15's reach · **the country is
  upper-cased once at the boundary** or the column's CHECK turns a lower-case
  write into an unmapped 23514 · **a NO-OP writes NO audit row** (a console sends
  back every field it drew) while still answering 200 (:12227 L-3) · **the lock
  is for the AUDIT ROW, not the write** — :14174 L-4's "warranted by the
  CONSEQUENCE", the consequence being the record of who changed a gym's billing
  country · **the meta names WHICH fields moved and their BEFORE values**, with
  `currencyDisplay` listed in its own right rather than inferred from `country`.
  **THE AUDIT IS THE PART TO READ — THREE INSTRUMENT FINDINGS, ALL MINE.**
  **(1) The first backfill test COULD NOT FAIL**: "no owner is missing it" comes
  back empty on a database with no owner rows, and the local Postgres has
  `owner_rows=0` — :5104 F5 and :18652's C/H-3 (*a verdict that depends on which
  database you point it at is worse than a missing one*). Closed :18652's way,
  **the test BUILDING ITS OWN LEGACY ROW** and running the backfill **read out of
  the shipped migration file**, never a re-typed copy (:12227). **Its first run
  FAILED on `startsWith("UPDATE")` — the statement is preceded by its comment
  block — and that failure is the proof it reads the file.** **(2) TWO OF EIGHT
  NEW MUTANTS WOULD HAVE COME BACK ALIVE, both holes in the TESTS: O120** named a
  fixture whose country is already upper-case, **O114** (the gym id leaving the
  edit's WHERE) named the authz test **whose every PATCH is refused before it
  reaches the repo** — the FOURTH `gym_id` predicate on this table family to ship
  unobserved (:14493 C/H-1, :15259 L-1). Closed with a successful owner edit plus
  the other gym as control. **:11846's two halves, and it was the FILTER half
  both times.** **(3) The whole-table pre-check ABORTED the sweep TWICE before a
  byte was written**, both my own edits moving somebody else's anchor: O15's line
  moved into `resolveCurrency` (re-anchored, **re-measured RED**), then O20's
  timezone anchor matched TWICE because the edit schema repeats the create
  schema's proof — caught by :15259 L-2's uniqueness guard. **Re-aiming O20 would
  have been the WRONG fix** (:15770 — a mutant is a claim about ONE call site;
  :14174 L-1's "a control that looks covered because a mutant was written for its
  sibling"); O20 keeps the create door and **O122 is its sibling on the edit
  door**, which matters more here because the route that could write a junk zone
  is the one built to FIX one.
  **PROVE, all LOCAL (:13659): `orgs.routes` 116/116 (+12) · `db.migration` 10/10
  (+2) · both in ONE invocation 126/126 · shared 51/51 · four more suites 69/69 ·
  tsc exit 0 AND PROVEN REAL by planting a type error · eslint clean on eight
  files · SWEEP a stated SUBSET of 122: 11 mutants · 11 RED · 0 ALIVE · 0 never
...
- **:30094** — 2026-09-02 — **THE GYM'S DAY GETS WRITTEN DOWN: the first writer
  `org_daily_stats` has ever had, the Overview's numbers on top of it, and a
  duplicate mutant id the harness could not see.** §4a of
  `CARD-gym-overview-numbers.md`, approved by Kd with migration `0020` reviewed
  as SQL first. **Read before touching `modules/orgs/rollup.ts` or
  `getOrgOverview`, before adding a job to the `rollups` queue, before reading
  `org_daily_stats` for anything a screen draws, before writing a backtick
  inside a `sql` template literal, before numbering a new mutant from the ids at
  the END of `mutate-orgs.mjs`, and before assuming the gym's numbers come from
  the nightly table.**
  **THE ROUTE READS `gym_attendance` LIVE AND THE PLAN SAID OTHERWISE** — a
  distinct count cannot be summed across days, and a nightly table is partial
  for part of every day (:5807). The rollup is still written: the workout join
  is Reports' source and too expensive per page load, and `gym_attendance` is on
  the UNRULED half of the DPDP list, so the day it becomes deletable the chart's
  source moves onto the aggregate. **`org_daily_stats` therefore has a WRITER
  and still no READER — do not quote it as feeding the Overview.**
  Hourly schedule, `EXTRACT(HOUR … AT TIME ZONE g.timezone) = 2` inside the job
  (no single UTC time is 02:00 everywhere) · seven finished days per run for the
  offline queue (R10.3) · **today is never written** · `attendance.read` and not
  `members.read` · the membership interval kept though redundant, because staff
  marking is *"not now"* and not never.
  **THE AUDIT'S REUSABLE HALF IS MINE: a DUPLICATE MUTANT ID that every guard in
  the harness passed** — the table is not in id order, `O217` already existed 900
  lines above, and the sweep ran two different mutants under one id with nothing
  red. Rows renumbered O228–O238 and **a duplicate-id guard added and proven by
  causing it**. Also: **a backtick inside a `sql` template literal silently ends
...
- **:33334** — 2026-09-04 — **T3 ROUND 1 ON BOTH OF TODAY'S CARDS (`:32929` the
  fold, `:33091` the greeting): ONE Critical/High, THE PACKET DOES NOT SHIP.**
  **Read before removing a surface a smoke sheet names, before re-pointing any
  smoke step at a different screen, before writing a second absence assertion
  into a suite that already has one, before adding a `truncate` to a flex child,
  and before adding an `OWED.md` line for a dead mutation anchor without first
  grepping `OWED.md` for it.** **C/H-1: `showMemberships={false}` took the
  membership card off the dashboard and the opening times ride on that row, so
  FOUR sheets sent the tester to a screen the app no longer draws** —
  `smoke-gym-hours-fold.md` 1–3 (the live UNRUN gate), `smoke-opening-hours.md`
  22/22b/23, and `smoke-join-door.md` 12, **which is recorded PASSED, so a false
  ✅ sat inside cited evidence.** **:32498 and :33265 §1 a third time, in the most
  literal shape: a step naming the APP's unreachable state, then the ACCOUNT's,
  now the SCREEN's** — and `:32583` §2 was applied correctly by `e807340` and not
  an hour later by `0fcfcba`. **STANDING: a commit that REMOVES a surface greps
  `RUNBOOK/` for the words on it before it lands.** All re-pointed at `My Gyms`,
  after CHECKING the calendar really folds and the nav item really says
  `My Gyms` — a re-point onto a control that does not fold would have been the
  same defect inside its own correction. **Four Low, all fixed: a FIFTH liar of
  the shape `:32929` §3 said it had found all four of** (`C214` failed 2 of 14
  before the added assertion, 3 of 14 after) · a comment struck in place
  (`:20587`) · **the greeting's missing `truncate`, where the review's fix was
  HALF of it** — a flex child needs `min-w-0` or `truncate` is inert (`:25567`) ·
...
- **:15260** — 2026-08-22 — **THE ROSTER BADGE, T3 ROUND 1: ZERO Critical/High —
  ...
  smoke by step number, and before repeating this card's §2.4 argument.** **SIX
  LOW, ALL FIXED, and FIVE OF THE SIX ARE THE RECORD RATHER THAN THE CODE** —
  four of those five being one defect, **a claim that outran what was measured**.
  **L-1 is the one with teeth and it is code: the roster's cross-gym predicate had
  NO observer, the FOURTH such predicate on this table to ship untested** —
  :14401 round 2 wrote O88–O90 saying "round 1's three fixes added three `gym_id`
  predicates and NOT ONE had a test", and this card added a fourth. Measured:
  delete it and all 92 tests stay green, **the both-ends test included**, which
  observes one of the two axes the copies can drift on. A payer at gym B holding
  keys at gym A would be badged Complimentary on B's roster and lose their Remove
  button — false on screen AND a money undercount, across tenants. Closed on the
  existing cross-gym fixture with gym A's roster as the control; mutant **O94**.
  **INSTRUMENT, MINE: my first probe of L-1 came back RED and the red was a
  5-second TIMEOUT on an unrelated test** — it would have "disproved" a true
  finding, and the unnamed cause pointed the flattering way (:11846). **L-2: this
  card made O88's anchor AMBIGUOUS** (one match at `3950a5d`, two after, the
  roster's 14-space line containing the 12-space prefix) **while three documents
  called it verified — a claim generalised from five measured anchors.** Census of
  all 93 found four ambiguous, three pre-existing. **Fixed with the guard :14493
  NAMED AND NEVER BUILT: the pre-check now refuses any anchor matching more than
  once**, three legacy rows in a shrink-only allow-list with an `OWED.md` line,
  **the guard proven to abort by removing one of them.** **L-3 is :5748 catching
  the chat in the act one paragraph after quoting it** — :15187's own correction
  was applied to `DECISIONS.md` and the sheet and missed in `HANDOFF.md` and this
  index, both still claiming a post-commit `git status` check that had not
  happened. **L-4: the smoke was cited by step numbers that do not match the
  sheet** — he ran a 7-step compressed version typed in CHAT, not the 8-step
  sheet; mapping now written out, **sheet step 2 named as UNRUN rather than
  counted** (implicitly covered: `readThrough` throws on a contract mismatch, so a
  roster that failed to parse would have shown no list), sheet committed. **L-5:
  the §2.4 argument was UNSOUND as written and the outcome still right** —
  `complimentary === false && takesSeat === false` means exactly "staff here", an
  EXACT inference, and trainers/managers hold `members.read` while `staff.manage`
  (which gates the staff LIST) is the owner's alone, so the roles refused that
  list can derive it. **Disclosure ACCEPTED and recorded, not reversed**; **must
  NOT be closed by withholding the field from trainers — that trades a disclosure
  for a FALSEHOOD** (:5807), and hiding it is Kd's ruling with its own card and
  `OWED.md` line. **L-6: the both-ends test is described doing something it does
  not do** — the cap assertions are compared to LITERALS, not derived from the
  roster's count; test sound, account wrong. **PROVE: orgs.routes 92/92 exit 0**
  (unchanged — L-1's assertions went into an EXISTING test, so the count does not
  move; a draft of this line said "93/93 (+1)", **this round's own finding
  recurring in the line recording it**) **· web 1057/1057 · shared 48/48 · build ✓
  · tsc + eslint clean · API SWEEP a
  stated SUBSET, O88 re-anchored + O92/O93/O94, 4 mutants · 4 RED · 0 ALIVE · 0
...
- **:15198** — 2026-08-22 — **THE ROSTER-BADGE SMOKE PASSES — on live data, and
  ...
  older-API fallback, and before editing any of the card's seven source files.**
  **"all passed".** **He ran a 7-step compressed version typed in CHAT, not the
  8-step sheet** (T3 L-4 caught the two numberings; mapping in the entry). Chat
  step 5 = sheet 6 is the subject and passed: an appointed trainer comes back to
  Members carrying the **Complimentary** badge with Remove correctly absent.
  **Chat 2/3/7 (sheet 3/4/8) are the CONTROLS that stop "badge everybody"
  passing.** **Chat 6 (sheet 7) is :14401's C/H-1 outside a fixture: the
  join-code count did not move on a promotion, on live data.** **Sheet step 2 was
  swallowed by the compression and is named as unrun** — implicitly covered,
  because `readThrough` throws on a contract mismatch, so a roster that failed to
  ...
  **NOT settled: the expand-then-contract fallback** (no older API exists to
  point a browser at — carried by mutant C38 and its test alone) **and the §2.4
  widening argument**, which is a review question and not a browser one.
  **T3 IS THE REMAINING GATE; the OWED roster line does NOT tick.**

- **:15093** — 2026-08-22 — **THE ROSTER SAYS WHO HAS A FREE PLACE — and the
  audit found the guarantee had no observable subject.** Builds :14953; server +
  web, **no migration**. **Read before touching the Members roster's badge,
  before adding a field to `/v1/orgs/:gymId/members`, before writing
  `gym_members.complimentary` anywhere, and before writing a mutant aimed at a
  rule the OWNER is excluded by twice.** A trainer's row now carries the same
  **Complimentary** badge the owner's does, because the gym is not charged for
  their place; before this they looked exactly like somebody occupying a paid
  seat. **NOT closed by writing `complimentary` for staff** — that is :14401
  C/H-1, the column meaning "did not JOIN" with three readers acting on it — but
  by a separate derived `takesSeat`, computed by `claimSeat`'s own count rule.
  **The rule is written out TWICE on purpose** (a shared `sql` fragment is R3.8's
  forbidden shape, :14493 Low-2) and what stops the copies drifting is a test
  driving BOTH ends on ONE fixture (:14013). **Two claims in this entry's first
  draft were FALSE and T3 measured both** — the cap assertions are compared to
  LITERALS, not derived from the roster's count (L-6), and "every anchor matches
  exactly once" was generalised from a five-anchor sample while O88 had gone from
  one match to two because of this card's own new line (L-2). O88 re-anchored;
  **the pre-check now REFUSES any anchor matching more than once**, with three
  pre-existing rows in a shrink-only allow-list carrying an `OWED.md` line. **THE AUDIT'S SURVIVOR IS
  THE PART TO READ AND IT WAS MINE: O92 SURVIVED because the OWNER IS EXCLUDED
  TWICE** — delete `complimentary = false` and their staff row still reports the
  place free, so the assertion could not fail. **:14401's O3 one card later on
  the roster's copy of the same rule**, :11846's both-halves-moved, :5104 F5's
  shape; closed the way the door closed it, with a **comped member who is not
  staff**, and re-measured RED. Code right throughout, coverage absent —
  :15007's own three-times finding, recurring in the card quoting it. **The §2.4
  addition was ARGUED**: the key set is asserted exactly on purpose (:10010), the
  field says "this place is free" and never "this person is staff", and who holds
  the keys stays on the owner-only Staff screen. **`takesSeat` is OPTIONAL for
  :12660's expand-then-contract reason and the alternative was measured**: a
  required key blanks the WHOLE roster during a web-newer-than-API window, and
  defaulting to "takes a seat" strips the OWNER's badge and offers a Remove the
  server refuses; absent, the client falls back to `complimentary` (its own test,
  its own mutant C38). Decisions not to re-derive: the badge keeps the word
  **Complimentary** · **no Remove beside a free place is the OLD rule** (the
  server refuses anybody still staff) · the web renders and never computes
  (R3.1) · **`joinedCount` and the `max_uses` gate are UNTOUCHED**. **PROVE: web
  1057/1057 exit 0 (+7) · shared 48/48 · orgs.routes 92/92 exit 0 (+1) · build ✓ ·
  tsc + eslint clean on seven files · WEB SWEEP 54 mutants · 54 RED · 0 ALIVE · 0
  never ran, exit 0.** **THE API SWEEP IS A SUBSET AND SAYS SO — 9 of 93**
  (O92/O93 plus every mutant in the two functions edited); first run **8 RED · 1
  ALIVE**, then O92/O93 re-measured **2 · 2 RED** after the fixture fix, the other
  seven standing on untouched source (:10726). **The whole-table anchor pre-check
  ran over all 93 both times and passed.** A full sweep was started and stopped at
  ~4 hours' projected cost, the stop verified by process list and `git status`
  rather than the tool's word. **NOTHING TICKS — SMOKE
